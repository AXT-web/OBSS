...
function addressmake() {
    ...
    var getsearch=document.getElementById('getsearch');
    URL = URL.split('?')[1]; //获取参数列表
    var BookTypeName=null;
    BookTypeName=decodeURIComponent(URL);
    var urlValue2='BookTypeName='+BookTypeName
    ...
    xhr2.onload = function () {
        ...
        if (res2.total == 0) {
            var booktypeid = res2.rows[0].booktypeid;
            var urlValue = 'booktypeid=' + booktypeid;
        }else {
            console.log('失败')
        }
    }
...
function onload2() {
    ...
    var BookTypeName=null;
    BookTypeName=decodeURIComponent(URL);
    var urlValue='BookTypeName='+BookTypeName
    console.log(urlValue);
    var xhr = new XMLHttpRequest();
    xhr.open('post', 'http://localhost:8080/OBSS_war_exploded/TbBooktype/selectname');
    // 设置请求参数格式的类型（post请求必须要设置）
    xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
    // 发送请求
    xhr.send(urlValue);
    ...
    xhr.onload = function () {
        let res = JSON.parse(xhr.responseText);
        if(res.total==0){
            var booktypeid=res.rows[0].booktypeid;
            var urlValue2 = 'booktypeid='+booktypeid;
            var xhr2 = new XMLHttpRequest();
            xhr2.open('post', 'http://localhost:8080/OBSS_war_exploded/TbBook/selectbooktypeid');
            // 设置请求参数格式的类型（post请求必须要设置）
            xhr2.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
            // 发送请求
            xhr2.send(urlValue2);
            ...
            xhr2.onload = function () {
                ...
                for (var i=0;i<res2.rows.length&&i<10;i++){
                    ...
                    var lastprice=parseFloat(res2.rows[i].bookprice)*0.8;
                    lastprice= parseInt(lastprice*100)/100;
                    serch_lastprice.innerHTML=lastprice;
                    setbookid.innerHTML=res2.rows[i].bookid;
                    search_top.style.display='block';
                    search_tid.href='detail.jsp?BookId='+res2.rows[i].bookid;
                    search_img.src='upload/'+res2.rows[i].bookid+'.jpg';
                    search_did.href='detail.jsp?BookId='+res2.rows[i].bookid;
                    search_name.innerHTML=res2.rows[i].bookname;
                    search_autor.innerHTML='作者：'+res2.rows[i].bookauthor;
                    search_baseprice.innerHTML='￥'+res2.rows[i].bookprice;
    ...
    function  lastnum(){
        ...
        xhr2.onload = function () {
            ...
            if (res2.total == 0) {
                ...
                var urlValue = 'booktypeid=' + booktypeid;
        var xhr = new XMLHttpRequest();
        xhr.open('post', 'http://localhost:8080/OBSS_war_exploded/TbBook/selectbooktypeid');
        // 设置请求参数格式的类型（post请求必须要设置）
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        // 发送请求
        xhr.send(urlValue);
